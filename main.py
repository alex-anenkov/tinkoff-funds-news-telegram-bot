...
def get_user_list_for_news_broadcast(ticker: str):
    db_ticker_name = ticker.upper() + "_ticker"
    db_cursor.execute("SELECT chat_id FROM Users WHERE " + db_ticker_name + " = ?", (1,))
    return db_cursor.fetchone()

def handle_news(titles, announces, links, ticker: str):
    # assert len(titles) == len(links)
    if len(titles) != len(links):
        print(f"{ticker}: len(titles) != len(announces)")
...
def get_web_page(url: str):
    ...
    try:
        ...
        wait = retries * 30
        print(f"Error getting web page. Wait {wait} sec and re-trying...")
        time.sleep(wait)
        retries += 1

def parse_web_and_insert_to_db(ticker: str):
    url = domain_name + "/invest/etfs/" + ticker + "/news/"
...
def handle_stop_msg(msg, res=False):
    bot.send_message(msg.chat.id, "Bye " + msg.chat.first_name)
